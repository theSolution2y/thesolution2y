...
2. Clone this repository and move to `theSolution` directory
   ```sh
   git clone https://github.com/theSolution
   cd theSolution
   ```
...
10. Setup Virtual Host
    1. For WAMP:
        ...
        - Go to `C:\wamp64\bin\apache\apache2.4.46\conf\extra\httpd-vhosts.conf` and add the following code snippet at the end of the file. Copy the absolute file path for the `public` directory of the project and paste it below where `your_project_path` is written. For example, your project path may look like `C:\wamp64\www\theSolution\public`.
            ...
            ```
        - Restart WAMP. Next, open this url in your browser: http://theSolution.test
    ...
    2. For XAMPP:
        ...
        - Go to `C:\xampp\apache\conf\extra\httpd-vhosts.conf` and add the following code snippet at the end of the file. Copy the absolute file path for the `public` directory of the project and paste it below where `your_project_path` is written. For example, your project path may look like `C:\xampp\htdocs\theSolution\public`.
            ...
            ```
        - Restart XAMPP. Next, open this url in your browser: http://theSolution.test
    ...
    3. For MAMP(macOS):
        ...
        - Go to `httpd.conf` file or edit this file in the terminal itself use this command
          ...
          - Open the `vhost` file in the terminal
            ...
            Copy the absolute file path for the `public` directory of the project and paste it below where `your_project_path` is written. For example, your project path may look like `/Application/MAMP/htdocs/theSolution/public`. Make sure you type '/public' at the end after your project path.
          ...
          - In case you are using Apache version 2.4 or above, the above code will give you a 403 Forbidden error. Make the following changes:
            ...
            ```
           - Restart MAMP. Next, open this url in your browser: http://theSolution.test
11. Login to the theSolution using the newly created user in the database. Go to `http://localhost/phpmyadmin/index.php` and search for the `users` table and you can find the user email in it. The default password to log in is `12345678`.
